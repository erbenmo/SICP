...
(define (install-rational-package)
  ...
  (define (drop x)
    (if (get 'project 'rational x)
	(let ((lower (get 'project 'rational (contents x)))
	      (upper (get 'raise 'scheme-number lower)))
	  (if (= x upper)
	      (drop lower)
	      x))
	x))
...
(define (install-complex-package)
  ...
  (define (drop x)
    (let ((lower (get 'project 'complex x))
	  (upper (get 'raise 'rational lower)))
      (if (= x upper)
	  lower
	  #f)))
